...
class PaymentController extends Controller
{
    ...
    public function get_checkout_id_apple_pay(Request $request)
    {
        ...
        $url = env('PAY_CHECKOUTS_URL', 'https://test.oppwa.com/v1/checkouts');
        $data =
            "entityId=" . env('APPLE_PAY_ENTITYID', '8ac7a4c8729db6f90172a323404c16f6') .
            "&amount=" . $request->price .
            "&currency=SAR" .
            "&paymentType=DB" .
            "&testMode=".
            "&shopperResultUrl=com.wisyst.mjrb.payments";

        try {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                'Authorization:Bearer ' . env('APPLE_PAY_AUTHORIZATION', 'OGFjN2E0Y2E2ZDA2ODBmNzAxNmQxNGM1NzMwYzE2ZDR8QVpZRXI1ZzZjZQ==')));
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, env('PAY_SSL', false));// this should be set to true in production
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    public function checkPaymentStatus_apple_pay(Request $request)
    {
        ...
        $url = env('PAY_BASE_URL', 'https://test.oppwa.com/');
        $url .= $request->resource;
        $url .= "?entityId=" . env('APPLE_PAY_ENTITYID', '8ac7a4c8729db6f90172a323404c16f6');
