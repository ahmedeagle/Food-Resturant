            ...
            <div class="col-lg-9 col-md-8 col-12 mt-4 mt-md-0 font-body-bold">
                                        ...
                                        <div class="media-body">
                                            ...
                                            <span class="d-block d-lg-inline">
                                                {{trans('site.order_num')}} <span
                                                        class="order-number">{{ $order->order_code }}</span>
                                            </span>
                                ...
                                <div class="p-3 rounded-lg shadow-around mt-4">
                                    ...
                                    <div class="media align-items-center flex-column flex-lg-row">
                                        ...
                                        <div class="media-body">
                                            ...
                                            <p class="text-gray font-body-md mb-0">
                                            <span class="d-block d-lg-inline">
                                                   {{trans('site.reservation_num')}} <span
                                                        class="reservation-number">{{ $reservation->reservation_code }}</span>
                                            </span>
                                                <span class="d-block d-lg-inline">
                                                <span class="d-none d-lg-inline">|</span>
                                                 {{trans('site.person_num')}}    : <span
                                                            class="reservation-person">{{ $reservation->seats_number }}</span>
                                            </span>
                                                <span class="d-block d-lg-inline">
                                                <span class="d-none d-lg-inline">|</span>
                ...
                <div class="reservation-section mt-5">
                    ...
                    <div class="py-2 pr-3 rounded-lg shadow-around bg-white">
                        <h4 class="page-title font-body-bold">{{trans('site.congestion_status')}}</h4>
                    </div>

                    <div class="p-3 rounded-lg shadow-around mt-4">

                        <form action="{{ url(LaravelLocalization::getCurrentLocale().'/restaurant/congestion') }}"
                              method="POST" class="new-kind-form multi-forms">
                        {{ csrf_field() }}<!---->
                            <div class="form-group">
                                <label for="congestion-status">{{trans('site.congestion_status')}} </label>
                                <select class="custom-select text-gray font-body-md border-gray" id="congestion-status"
                                        name="congestion-status" required="">
                                    <option value="">{{trans('site.choose_congestion_status')}} </option>
                                    @if(isset($congestion) && $congestion -> count() > 0 )
                                        @foreach($congestion as $c)
                                            <option value="{{ $c->id }}"
                                                    @if($c->id == $branch->congestion_settings_id) selected @endif>{{ $c->name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            <button type="submit"
                                    class="add-meal-btn btn btn-primary py-2 px-5">{{trans('site.confirm')}}</button>
                        </form><!-- .new-kind-form -->
                    </div>
                </div><!-- .col-* -->
